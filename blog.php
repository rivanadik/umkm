...
<div class="container bg-white rounded pt-4">
    ...
    <div class="row text-left mb-5">
        <?php
                            $query="SELECT * FROM tbl_pos a JOIN tbl_kat_pos m ON a.id_kategori=m.id_kategori ORDER BY Gambar asc LIMIT 1";
                            $result=mysqli_query($db,$query);
                            while ($data = mysqli_fetch_array($result)) {
                                $tgl = $data['tgl'];
                        ?>
        ...
        <?php } ?>
            ...
            <div class="row">
                <div class="col">
                    ...
                    <hr align="left">
                    <?php
                    $query = "SELECT MAX(a.tgl) as tgl, m.nm_kategori, COUNT(a.id_kategori) as count FROM tbl_pos a JOIN tbl_kat_pos m ON a.id_kategori=m.id_kategori GROUP BY a.id_kategori ORDER BY tgl asc LIMIT 5";
                    $result = mysqli_query($db, $query);             
                    if ($result) {
                        while ($data = mysqli_fetch_array($result)) {
                            ?>
                                    <ul class="list-group">
                                        <li class="list-group-item d-flex justify-content-between align-items-center"
                                            style="background-color: transparent;"><?= $data['nm_kategori']; ?>
                                            <span class="badge badge-success badge-pill"><?= $data['count']; ?></span>
                                        </li>
                                    </ul>
                        <?php
                        }
